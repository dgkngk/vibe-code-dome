...
def get_workspaces(db: Session, user_id: int):
    return db.query(models.Workspace).filter(or_(models.Workspace.owner_id == user_id, models.Workspace.id.in_(
        db.query(models.workspace_members.c.workspace_id).filter(models.workspace_members.c.user_id == user_id)
    ))).all()
...
def get_workspace_by_id(db: Session, workspace_id: int, user_id: int):
    workspace = db.query(models.Workspace).filter(
        ...
        or_(models.Workspace.owner_id == user_id, models.Workspace.id.in_(
            db.query(models.workspace_members.c.workspace_id).filter(models.workspace_members.c.user_id == user_id)
        ))
    ).first()
    if workspace:
        workspace.members = db.query(models.User).join(models.workspace_members).filter(
...
def add_member_to_workspace(db: Session, workspace_id: int, user_id: int):
    if not db.query(models.workspace_members).filter(
        models.workspace_members.c.workspace_id == workspace_id,
        models.workspace_members.c.user_id == user_id
    ).first():
        ins = models.workspace_members.insert().values(workspace_id=workspace_id, user_id=user_id)
        db.execute(ins)
        db.commit()
    return True
...
def get_members(db: Session, workspace_id: int):
    return db.query(models.User).join(models.workspace_members).filter(
        models.workspace_members.c.workspace_id == workspace_id
    ).all()
...
def update_card(db: Session, card_id: int, list_id: int, card_update: schemas.CardUpdate, user_id: int):
    ...
    if not current_workspace or not (current_workspace.owner_id == user_id or db.query(models.workspace_members).filter(
    ...
    if card_update.list_id and card_update.list_id != list_id:
        new_list = db.query(models.List).filter(models.List.id == card_update.list_id).first()
        if not new_list:
            return None
        ...
        if not new_workspace or not (new_workspace.owner_id == user_id or db.query(models.workspace_members).filter(
