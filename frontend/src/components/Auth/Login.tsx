  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900">
      <form onSubmit={handleSubmit} className="max-w-md w-full mx-4 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        <h2 className="text-2xl font-bold mb-4 text-center text-gray-900 dark:text-gray-100">{t('login.title')}</h2>
        <input
          type="email"
          value={email}
        ...
        <p className="mt-4 text-center text-gray-900 dark:text-gray-100">
          {t('no.account')} <Link to="/register" className="text-primary dark:text-primary">{t('register.link')}</Link>
        </p>
      </form>
    </div>
  );
